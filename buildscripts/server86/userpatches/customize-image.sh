...
Main() {
	case $RELEASE in
		...
		bookworm)
			...
			apt install bash-completion alsa-utils apt-utils sudo firmware-sof-signed alsa-firmware-loaders cifs-utils vainfo vdpauinfo firmware-nvidia-gsp pciutils glmark2* -y
			apt autoremove -y
		        apt purge snapd -y
            		echo 'Package: snapd' > /etc/apt/preferences.d/nosnap.pref
            		echo 'Pin: release a=*' >> /etc/apt/preferences.d/nosnap.pref
