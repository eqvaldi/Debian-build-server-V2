...
Main() {
	case $RELEASE in
		...
		bookworm)
			...
			apt install build-essential dialog git nasm libgl1-mesa-dev libsdl2-dev flac libflac-dev libvpx-dev libgtk2.0-dev freepats ninja-build qtbase5-dev qtbase5-private-dev qtbase5-dev-tools qttools5-dev -y
			apt install bash-completion alsa-utils apt-utils sudo libcurl4-openssl-dev firmware-sof-signed alsa-firmware-loaders libsdl2-net-dev cifs-utils vainfo vdpauinfo firmware-nvidia-gsp pciutils spek glmark2* -y
			apt autoremove -y
		        apt purge snapd -y
            		echo 'Package: snapd' > /etc/apt/preferences.d/nosnap.pref
            		echo 'Pin: release a=*' >> /etc/apt/preferences.d/nosnap.pref
		...
		trixie)
			...
			apt install build-essential dialog git nasm libgl1-mesa-dev libsdl2-dev flac libflac-dev libvpx-dev libgtk2.0-dev freepats ninja-build qtbase5-dev qtbase5-private-dev qtbase5-dev-tools qttools5-dev libzip-dev zipcmp zipmerge ziptool libsdl2-mixer-dev bash-completion alsa-utils apt-utils sudo libcurl4-openssl-dev firmware-sof-signed alsa-firmware-loaders libsdl2-net-dev cifs-utils vainfo vdpauinfo firmware-nvidia-gsp pciutils spek glmark2* -y
			apt autoremove -y
		        apt purge snapd -y
            		echo 'Package: snapd' > /etc/apt/preferences.d/nosnap.pref
            		echo 'Pin: release a=*' >> /etc/apt/preferences.d/nosnap.pref
